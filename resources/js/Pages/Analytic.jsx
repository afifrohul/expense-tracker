...
export default function Analytic({ auth, all_years }) {
    ...
    useEffect(() => {
        const fetchExpensesCategory = async () => {
            try {
                const response = await axios.get(`/api/expenses-by-category?month=${pie_month}&year=${pie_year}`);
                setPieData(response.data.map(item => ({ ...item, total: Number(item.total) })));
            } catch (error) {
                console.error("Error fetching data", error);
            }
        };
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-white shadow-md p-2 rounded">
                    ...
                    <p className="text-blue-500">{formatRupiah(payload[0].value)}</p>
                </div>
            );
        }
        return null;
            ...
            <div className='flex gap-4'>
                <div className='flex-1 flex flex-col gap-4'>
                    <div className='border border-slate-300 p-4 rounded-lg w-full'>
                        ...
                        <ResponsiveContainer width="100%" height={300} className='p-3 w-full'>
                    ...
                    <div className='flex gap-4'>
                        <div className='border border-slate-300 p-4 rounded-lg w-fit'>
                            <div className='mb-2'>
                                <p className='font-bold text-lg'>Data pengeluaran berdasarkan kategori</p>
                                <hr className='mt-2'/>
                            </div>
                            <div className='flex gap-4'>
                                <div className='w-full'>
                        ...
                        <div className='border border-slate-300 p-4 rounded-lg w-full'>
                            <div className='mb-2'>
                                <p className='font-bold text-lg'>Data pemasukan dan pengeluaran per tahun</p>
                                <hr className='mt-2'/>
                            </div>
                            <div className="flex ">
                            <ResponsiveContainer width="100%" height={400} className="p-3 w-full" >
                                <BarChart data={yData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis dataKey="year" />
                                    <YAxis />
                                    <Tooltip formatter={(value) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(value)} />
